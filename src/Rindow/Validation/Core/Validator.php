...
use Interop\Lenient\Validation\Validator as ValidatorInterface;
...
class Validator implements ValidatorInterface
{
    ...
    const GROUPSEQUENCE_CLASS_NAME  = 'Rindow\\Validation\\Constraints\\GroupSequence';
        ...
        ConstraintValidatorFactoryInterface $constraintValidatorFactory=null)
    {
        ...
        if($constraintManager)
            ...
            $this->constraintManager = new ConstraintContextManager($constraintValidatorFactory,null,$annotationReader);
    }
    ...
    {
        return $this->constraintManager->getAnnotationReader();
    }
    ...
    protected function determineValidationOrder($groups,$constraints)
    {
        ...
        if(isset($constraints['__CLASS__'][self::GROUPSEQUENCE_CLASS_NAME])) {
            if($groups==array('Default'))
                $groups = $constraints['__CLASS__'][self::GROUPSEQUENCE_CLASS_NAME];
        }
    ...
    protected function getValueFromProperty($object,$propertyName)
    {
        if(method_exists($object, $propertyName)) {
            ...
            if($object instanceof PropertyAccessPolicy) {
                $value = $object->$propertyName;
            } else {
